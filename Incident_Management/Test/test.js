...
axios.defaults.auth = { username: 'fahath' }

jest.setTimeout(11111)

describe('Test The GET Endpoints', () => {
  it('Should check Processor Service', async () => {
    const processorService = await cds.connect.to('SupportService')
    const { Incident } = processorService.entities
    expect(await SELECT.from(Incident)).to.have.length(4)
  })
  ...
  it('Test Expand Entity Endpoint', async () => {
    const { data } = await GET(`/odata/v4/support/Customers?$select=firstName&$expand=incidents`)
...
describe('Draft Choreography APIs', () => {
  ...
  it('Create an incident ', async () => {
    const { status, statusText, data } = await POST(`/odata/v4/support/Incident`, {
  ...
  it('+ Activate the draft & check Urgency code as H using custom logic', async () => {
    const response = await POST(
      `/odata/v4/support/Incident(ID=${draftId},IsActiveEntity=false)/SupportService.draftActivate`
    )
  ...
  it('+ Test the incident status', async () => {
    const {
      status,
      data: { status_code, ID }
    } = await GET(`/odata/v4/support/Incident(ID=${draftId},IsActiveEntity=true)`)
    incidentId = ID
  ...
  describe('Close Incident and Open it again to check Custom logic', () => {
    it(`Should Close the Incident-${draftId}`, async () => {
      const { status } = await POST(
        `/odata/v4/support/Incident(ID=${incidentId},IsActiveEntity=true)/SupportService.draftEdit`,
        {
    ...
    it(`Should Close the Incident-${draftId}`, async () => {
      const { status } = await PATCH(`/odata/v4/support/Incident(ID=${incidentId},IsActiveEntity=false)`, {
        status_code: 'C'
      })
    ...
    it('+ Activate the draft & check Status code as C using custom logic', async () => {
      const response = await POST(
        `/odata/v4/support/Incident(ID=${incidentId},IsActiveEntity=false)/SupportService.draftActivate`
      )
    ...
    it('+ Test the incident status to be closed', async () => {
      const {
        status,
        data: { status_code }
      } = await GET(`/odata/v4/support/Incident(ID=${incidentId},IsActiveEntity=true)`)
      expect(status).to.eql(200)
  ...
  it('- Delete the Draft', async () => {
    const response = await DELETE(`/odata/v4/support/Incident(ID=${draftId},IsActiveEntity=false)`)
    expect(response.status).to.eql(204)
  })
  it('- Delete the Incident', async () => {
    const response = await DELETE(`/odata/v4/support/Incident(ID=${draftId},IsActiveEntity=true)`)
    expect(response.status).to.eql(204)
  })
})
